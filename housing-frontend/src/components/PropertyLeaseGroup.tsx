...
interface Props {
  propertyName: string;
  leases: Lease[];
  // Callback to tell the parent (Leases.tsx) to switch to the Profile View
...
const PropertyLeaseGroup: React.FC<Props> = ({ propertyName, leases, onViewProfile }) => {
  ...
  // Extract property location from the first lease in the group
        ...
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-gray-400 text-[10px] font-black uppercase tracking-[0.15em] mt-1">
              {location} • {leases.length} Active Agreements
            </p>
          </div>
        </div>
        ...
        <div className="border-t border-gray-50 overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[900px]">
            ...
            <tbody className="divide-y divide-gray-50">
              {leases.map((lease) => (
                <tr key={lease.id} className="hover:bg-blue-50/20 transition-colors group">
                  {/* Unit Number */}
                  <td className="px-8 py-5 font-black text-blue-600">
                    <span className="bg-blue-50 px-3 py-1 rounded-lg text-sm">
                      #{lease.unit.unitNumber}
                    </span>
                  </td>

                  {/* Tenant Details with Profile Link */}
                  ...
                  {/* Dates */}
                  <td className="px-8 py-5">
                    <div className="text-xs font-bold text-gray-600 flex flex-col gap-0.5">
                      <span>Start: {new Date(lease.startDate).toLocaleDateString()}</span>
                      <span className="text-blue-400">
                        End: {lease.endDate ? new Date(lease.endDate).toLocaleDateString() : "Ongoing"}
                      </span>
                    </div>
                  </td>
                    ...
                    <div className="flex justify-end gap-2">
                      {lease.status === 'active' && (
                        <button 
                          ...
                          className="flex items-center gap-1.5 px-3 py-2 bg-amber-50 text-amber-600 rounded-xl hover:bg-amber-100 transition-all border border-amber-100 shadow-sm"
                        >
                      ...
                      <button 
                        ...
                        className="flex items-center gap-1.5 px-3 py-2 bg-red-50 text-red-500 rounded-xl hover:bg-red-500 hover:text-white transition-all border border-red-100 shadow-sm"
                      >
