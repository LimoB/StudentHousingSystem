import { useEffect, useMemo } from "react"; // Added useMemo for stable filtering
...
const PropertyDetail: React.FC = () => {
  ...
  // Use Memoized filtering to ensure we catch all variations of ID keys
  const filteredUnits = useMemo(() => {
    // Priority 1: Check if the property object itself has nested units (from the fetchPropertyById response)
    if (selectedProperty?.units && Array.isArray(selectedProperty.units)) {
      return selectedProperty.units;
    }

    // Priority 2: Filter from the global units array (handling string vs number)
  ...
  useEffect(() => {
    if (id && token) {
      ...
      // Clear old data so we don't flash previous property units
      dispatch(resetUnits()); 
          ...
          <div className="lg:col-span-2 space-y-10">
            ...
            <section className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
              ...
              <p className="text-slate-500 leading-relaxed text-lg font-normal">
                {selectedProperty.description || "Premium accommodation near university campus."}
              </p>
            </section>
            ...
            <section className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-medium text-slate-400">{filteredUnits.length} units available</span>
              </div>
              ...
              {filteredUnits.length > 0 ? (
                    ...
                    <div key={unit.id} className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm transition-all hover:shadow-md group">
                      <div className="flex justify-between items-start mb-6">
                        ...
                        <span className={`px-3 py-1 text-[10px] font-bold rounded-full border ${
                          (unit.isAvailable === true || unit.isAvailable === 1 || unit.isAvailable === "true") 
                          ? "bg-emerald-50 text-emerald-600 border-emerald-100" 
                          : "bg-slate-50 text-slate-400 border-slate-200"
                        }`}>
                          {(unit.isAvailable === true || unit.isAvailable === 1 || unit.isAvailable === "true") ? "AVAILABLE" : "OCCUPIED"}
                        </span>
                      </div>
                      ...
                      <button
                        disabled={!(unit.isAvailable === true || unit.isAvailable === 1 || unit.isAvailable === "true")}
                      ...
                      >
                        {(unit.isAvailable === true || unit.isAvailable === 1 || unit.isAvailable === "true") ? <>Secure this Unit <ArrowRight size={16}/></> : "Not Available"}
                      </button>
                    </div>
                  ))}
                </div>
                ...
                <div className="bg-white p-16 rounded-[2.5rem] border-2 border-dashed border-slate-100 flex flex-col items-center text-center">
                   ...
                   <h4 className="text-lg font-bold text-slate-900">No units found</h4>
                   <p className="text-slate-400 max-w-xs mx-auto">We couldn't find any rooms for this property. If you believe this is an error, please contact the landlord.</p>
                </div>
              )}
            </section>
          </div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-10 space-y-6">
              <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
                <h4 className="font-bold text-slate-900 text-lg mb-6">Quick Overview</h4>
                <div className="space-y-5">
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-slate-400 font-medium flex items-center gap-2"><Calendar size={16}/> Date Listed</span>
                    <span className="font-bold text-slate-700">
                        {selectedProperty.createdAt ? new Date(selectedProperty.createdAt).toLocaleDateString() : 'Active'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-slate-400 font-medium flex items-center gap-2"><Building2 size={16}/> Property Type</span>
                    <span className="font-bold text-slate-700">Student Residence</span>
                  </div>
                  <div className="pt-5 border-t border-slate-50">
                    <div className="bg-blue-50/50 p-4 rounded-2xl border border-blue-100/50">
                      <p className="text-[11px] text-blue-600 font-bold uppercase tracking-wider mb-1">Support</p>
                      <p className="text-xs text-slate-500 leading-relaxed">Need help? Contact university housing for assistance with bookings.</p>
                    </div>
                  </div>
                </div>
              </div>
