...
import { CreditCard, ExternalLink, Clock, CheckCircle2, AlertCircle, ReceiptText } from "lucide-react";

const Payments: React.FC = () => {
  const dispatch = useDispatch<AppDispatch>();
  
  // 1. Get payments from state
  const { payments: allPayments, loading, error } = useSelector((state: RootState) => state.payments);
  ...
  const studentPayments = useMemo(() => {
    return allPayments.filter((p) => Number(p.studentId) === Number(currentUserId));
  ...
  if (loading) return (
    <div className="p-20 text-center flex flex-col items-center">
      ...
      <p className="text-gray-500 font-bold text-lg">Syncing your transactions...</p>
    </div>
  );
    ...
    <div className="p-6 max-w-6xl mx-auto min-h-screen">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-500 font-medium">Manage and track your M-Pesa housing payments</p>
        </div>
        <div className="bg-blue-600 px-6 py-3 rounded-2xl shadow-lg shadow-blue-100 flex items-center gap-3 text-white">
            <ReceiptText size={20} />
            <span className="font-bold">{studentPayments.length} Transactions</span>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50/50 border-b border-gray-100">
                ...
                <th className="p-6 text-xs font-black uppercase tracking-widest text-gray-400">Amount</th>
                <th className="p-6 text-xs font-black uppercase tracking-widest text-gray-400">M-Pesa Receipt</th>
                <th className="p-6 text-xs font-black uppercase tracking-widest text-gray-400">Date</th>
                <th className="p-6 text-xs font-black uppercase tracking-widest text-gray-400 text-right">Details</th>
              </tr>
            </thead>
                  ...
                  <tr>
                      <td colSpan={5} className="p-20 text-center">
                        <div className="flex flex-col items-center opacity-40">
                          ...
                          <p className="text-gray-500 font-bold">No payments found in your account.</p>
                        </div>
                      </td>
                  </tr>
              ) : (
                  ...
                  <tr key={p.id} className="hover:bg-blue-50/30 transition-colors group">
                      <td className="p-6">
                        <div className="flex items-center gap-3">
                            <div className={`p-2 rounded-full ${
                              ...
                              {p.status === "paid" ? <CheckCircle2 size={16} /> : 
                               p.status === "pending" ? <Clock size={16} /> : <AlertCircle size={16} />}
                            </div>
                      ...
                      <td className="p-6 text-right">
                        <Link
                            ...
                            className="inline-flex items-center gap-2 bg-gray-900 text-white px-4 py-2 rounded-xl font-bold hover:bg-blue-600 transition-all text-sm shadow-sm opacity-0 group-hover:opacity-100"
                        >
